...
namespace NAssetNameGenerator
{
    public static class AssetName
    {
        private static Random _rng = new Random();
        private static List<string> _leftItems = new List<string>() {
                "admiring",
                "adoring",
                "affectionate",
                "agitated",
        ...
        private static List<string> _rightItems = new List<string>()
        {
        ...
        public static string NewName(int randomDigits = 0)
        {
            if (randomDigits <= 0)
            {
                return $"{_leftItems[_rng.Next(_leftItems.Count)]}_{_rightItems[_rng.Next(_rightItems.Count)]}";
            }

            StringBuilder digitSB = new StringBuilder(randomDigits);
            for (int i = 0; i < randomDigits; i++)
            {
                digitSB.Append(_rng.Next(0, 10).ToString());
            }
            return $"{_leftItems[_rng.Next(_leftItems.Count)]}_{_rightItems[_rng.Next(_rightItems.Count)]}_{digitSB.ToString()}";
        }
    }
}